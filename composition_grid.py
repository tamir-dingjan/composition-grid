...
def parse_args(args):
    ...
    parsed = my_parser.parse_args(args)
    return parsed
...
def process_config_file(wanted_lipids_file, old_grid):
...
def generate_grids(new_grid_path, old_grid_path, wanted_lipids_file_path):
    ...
    wanted_lipids, selected_lipids = process_config_file(wanted_lipid_file, old_grid)
    ...
    if len(wanted_lipids) > 0:
        ...
        for small_con in small:
            ...
            while current_mod_factor > 0:
                bigger_con = big[reg]
                # print("reg: " + str(reg) + " last: " + str(bigger_con) + "mod: " + str(current_mod_factor), "small: ", str(small_con))
                val_bool = []
...
def main():
    ...
    new_grid_path = parsed.New_Grid_Path
    old_grid_path = parsed.Data_Path
    wanted_lipids_file_path = parsed.Setup_Path

    # Create output location
    try:
        os.mkdir(new_grid_path)
    except FileExistsError:
        print("The output path specified already exists: %s" % new_grid_path)
        overwrite = input("Overwrite existing files? (y/n)")
        if overwrite == "n":
            sys.exit()
        elif overwrite != "y":
            raise ValueError("Invalid answer: %s" % overwrite)
    ...
    generate_grids(new_grid_path, old_grid_path, wanted_lipids_file_path)
